...
Type-specific options are passed via the `type_opts` list parameter.

The jittered sampling is a regular sample moved by an amount of noise. 
The `amount` option in `type_opts` controls the amount of jitter.
...
The outcome is controlled by the `nclusters` and `radius` options in `type_opts`, where the former controls the number of clusters and the latter the radius of the buffer around each cluster.
